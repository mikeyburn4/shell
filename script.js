document.getElementById("cmd-input").addEventListener("keydown", function(event) {
    if (event.key === "Enter") {
        ...
        if (input.toLowerCase() === 'help') {
            output.innerHTML += "Fake Terminal Commands Available: help, dir, cls, ping, gpupdate, netstat, tracert, nmap, whoami, ipconfig, shutdown /s /t 0, h4ck3rmode, tasklist, systeminfo\n";
        ...
        } else if (input.toLowerCase() === 'tasklist') {
            output.innerHTML += `Image Name                     PID Session Name        Session#    Mem Usage\n========================= ======== ================ =========== ============\nexplorer.exe                   1804 Console                    1    53,144 K\ncmd.exe                        3784 Console                    1    25,332 K\nchrome.exe                    10592 Console                    1   324,536 K\n\n`;
        } else if (input.toLowerCase() === 'systeminfo') {
            output.innerHTML += `Host Name:                 FAKE-TERMINAL\nOS Name:                   Microsoft Windows 10 Pro\nOS Version:                10.0.19042 N/A Build 19042\nOS Manufacturer:           Microsoft Corporation\nSystem Type:               x64-based PC\nProcessor(s):              1 Processor(s) Installed.\n                           [01]: Intel64 Family 6 Model 158 Stepping 10 GenuineIntel ~2592 Mhz\nBIOS Version:              American Megatrends Inc. 1502, 1/10/2021\n\n`;
        } else {
